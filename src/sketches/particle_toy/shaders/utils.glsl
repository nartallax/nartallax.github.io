...
uint encodeSignedFloat(float value, float range){
...
#define getFloatPairByCoords(texX, texY, coords, range) ((vec2(texture(texX, coords).x, texture(texY, coords).x) / INT_RANGEF) * range)
#define getSignedFloatPairByCoords(texX, texY, coords, range) (((vec2(texture(texX, coords).x, texture(texY, coords).x) - INT_RANGEF) / INT_RANGEF) * range)
#define getFloatPairByIndex(texX, texY, index, range) ((vec2(getUintPairByIndex(texX, texY, index)) / INT_RANGEF) * range)
